...
try {
    ...
    throw new Error(`Failed to retrieve restarant's foods: ${error.message}`);
}
...
return food;
    ...
    throw new Error(
        `Failed to update the availability status for food with ID ${foodId}: ${error.message}`
    );
}

},

async finfFoodById(foodId) {
